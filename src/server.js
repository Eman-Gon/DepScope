...
const {
  ...
  runScanCycle, startCron, stopCron, getCronStatus, generateDetailedReport,
...
app.get('/api/watchlist', (_req, res) => {
  const entries = getWatchlist().map(w => ({
    id: w.id,
    input: w.repoUrl,
...
app.post('/api/watchlist/scan', async (_req, res) => {
  ...
  const scan = await runScanCycle(alertPhone, (status, message) => {
...
<Response><Speak>No phone number available to send the report. Goodbye.</Speak></Response>`);
    ...
    // Send the detailed report (generated by Gemini), not just the short SMS summary
    let reportText = scan?.detailedReport;
    if (!reportText) {
      // Generate on-demand if not pre-generated
      try {
        reportText = await generateDetailedReport(scan?.results || []);
      } catch (_) {
        reportText = scan?.alertReport?.smsText || 'DepScope watchlist alert — check your dashboard for details.';
      }
    }
    const result = await sendSMS(targetPhone, reportText);
